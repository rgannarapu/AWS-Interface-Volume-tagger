...
def prepare_tags_for_volume(instance_tags):
    volume_tags = []
    ...
    if instance_tags != None:
        for each_tag in instance_tags:
            ...
            if each_tag['Key'].lower() == "name":
                temp_each_tags['Key'] = each_tag['Key']
                temp_each_tags['Value'] = each_tag['Value'] + "-volume"
                volume_tags.append(temp_each_tags)
            else:
                temp_each_tags['Key'] = each_tag['Key']
                temp_each_tags['Value'] = each_tag['Value']
                volume_tags.append(temp_each_tags)
...
def ec2_tags_to_volume(instance_list):
    ...
    for instance in instance_list:
        ...
        if len(volume_tags) > 0:
            attach_tags_to_volume(instance,volume_tags)
        else:
